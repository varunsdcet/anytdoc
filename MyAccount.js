 ...
 if (res.is_booking == 1){
   this.setState({visible:true})
     ...
     _fancyShareMessage=()=>{
             ...
             var a = 'Hey! Checkout Anytimedoc app. ' +'https://play.google.com/store/apps/details?id=com.anytimedoc' +' .Use my referral code ' + reefercode+ ' to get 150 wallet points.'
...
if (responseJson.user_details.weight != "" && responseJson.user_details.height != ""){
 var calculate = responseJson.user_details.height * responseJson.user_details.height
                    var bm = responseJson.user_details.weight/calculate
if (bm < 18.5){
  this.setState({bmi:'You are Underweight'})
  this.setState({check:'CLINICAL NUTRITIONIST'})
}else if (bm > 18.5 && bm < 24.9){
  this.setState({bmi:'You are Fit'})
    this.setState({check:'Medicine'})
}else if (bm > 25 && bm < 29.9){
  this.setState({bmi:'You are Overweight'})
    this.setState({check:'CLINICAL NUTRITIONIST'})
}else{
...
<View style = {{flexDirection:'row',width:window.width ,marginTop:20,marginBottom:20}}>

<Image style = {{width :100 ,height :100,marginLeft:14,marginTop:16,marginBottom:8,borderRadius:8}}
       ...
       <View style = {{flexDirection:'row'}}>
                              ...
                              <Text style={{fontSize:20,color:'#1E1F20',marginLeft:5,width:window.width -200,fontFamily:GLOBAL.bold,marginTop:30,marginLeft:30}}>
{this.state.name}
                              </Text>
                              <TouchableOpacity onPress={() => this.props.navigation.navigate('EditProfile')
                                        }>
                              <Image style = {{width :20 ,height :20,marginTop:35}}
                                     source={require('./edit.png')}/>
                                     </TouchableOpacity>


       </View>
       <Text style={{fontSize:13,color:'#1E1F20',marginLeft:5,fontFamily:GLOBAL.regular,marginTop:4,marginLeft:30,width:200}}>
{this.state.email}
       </Text>

         <TouchableOpacity onPress={() => this.props.navigation.navigate('Speciality',this.state.check)
                   }>

                   <View>
         <Text style={{fontSize:15,color:'#9393AA',marginLeft:5,fontFamily:GLOBAL.regular,marginTop:4,marginLeft:30}}>
  {this.state.bmi}
         </Text>
         {this.state.bmi != "" && (
           <Text style={{fontSize:15,color:'#9393AA',marginLeft:5,fontFamily:GLOBAL.regular,marginTop:4,marginLeft:30}}>
    Click and Consult Now
           </Text>
         )}
...
<Card style = {{marginLeft:30,width:window.width -60,borderRadius:8}}>
<FlatList style = {{marginLeft:5,width:window.width - 50,marginBottom:10}}
               ...
               <Card style = {{marginLeft:30,width:window.width -60,marginTop:30,borderRadius:8}}>
               <FlatList style = {{marginLeft:5,width:window.width - 50,marginBottom:10}}
...
<Image style = {{width :100 ,height :100,marginRight:34,marginTop:18}}
       source={require('./invite.png')}/>

       <View>
        <Text style={{fontSize:13,color:'#1E1F20',fontFamily:GLOBAL.semi,marginLeft:-30,marginTop:20,width:window.width -200}}> ₹150  is waiting. Invite and Earn.</Text>
        <Button
            style={{paddingTop:5,marginTop:14,fontSize: 18, color: 'white',backgroundColor:'#D90000',marginLeft:'5%',width:window.width/3 - 30,height:40,fontFamily:GLOBAL.semi,borderRadius:12,fontWeight:'bold'}}
